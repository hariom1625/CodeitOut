...
class QuestionList extends React.Component {
  ...
  componentDidMount() {
    axios
      .get(`${process.env.REACT_APP_SERVER}/api/question`, {
        headers: {
  ...
  render() {
            ...
            <div className="list-group">
              <Link to={`/questionlist/${question.problemCode}`}>
                <button
                  type="button"
                  onClick={(event) => {
                    this.onHandleClick(question.id);
                  }}
                  className=" prob-btn list-group-item list-group-item-action active"
                  aria-current="true"
                >
    ...
    if (this.state.loaded) {
      return (
        <div>
          <h1 className="ql-heading">Question List</h1>
          {que}
        </div>
      );
    } else {
